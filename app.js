...
function chatbotView() {
  return `<div class="section-head"><div><h2>Respostas e automações</h2><p>Cada resposta liga uma palavra-chave do WhatsApp a texto, template ou URL externa.</p></div><button class="btn primary" data-action="new-rule">+ Criar resposta</button></div><div class="stats">${stat('Respostas',state.rules.length,'Total cadastrado','♟')}${stat('Ativas',state.rules.filter(r=>r.active).length,'Respondendo agora','✓','blue')}${stat('URLs/Webhooks',state.rules.filter(r=>String(r.responseType).includes('URL')||String(r.responseType).includes('Webhook')).length,'Criam teste no provedor','⚡')}${stat('Chamadas de API','11','Hoje','⌁')}</div><article class="card"><div class="filters"><input class="input" placeholder="Pesquisar regra ou palavra-chave"><select class="select"><option>Todos os dispositivos</option><option>WhatsApp principal</option></select></div>${state.rules.map(ruleCard).join('')}</article>`;
}

function ruleCard(r) {
  return `<div class="rule-card"><div class="rule-top"><span class="automation-icon">${r.responseType?.includes('URL') || r.responseType?.includes('Webhook') ? '⚡' : '♟'}</span><div><h3>${safe(r.name)}</h3><p>Palavra-chave: <b>${safe(r.keyword)}</b> • ${safe(r.match)}</p></div>${badge(r.active)}<div class="mini-actions"><button data-action="edit-rule" data-id="${r.id}">Editar</button><button data-action="toggle-rule" data-id="${r.id}">${r.active?'Pausar':'Ativar'}</button></div></div><div class="rule-body"><div class="rule-step"><strong>Tipo de resposta</strong>${safe(r.responseType || 'Texto')}</div><div class="rule-step"><strong>URL / ação</strong>${safe(r.webhookUrl || r.action || 'Responder texto')}</div><div class="rule-step"><strong>Resposta ao cliente</strong>${safe(r.reply)}</div></div></div>`;
}

function testLinksView() {
  const links = state.testLinks || [];
  const listHtml = links.length
    ? `<div class="test-link-list">${links.map(t=>`<div class="test-link-row"><div><strong>${safe(t.packageName)}</strong><small>Palavra-chave que o cliente vai mandar: ${safe(t.keyword)} � M�todo: ${safe(t.method || 'POST')}</small><code>${safe(t.url)}</code></div><div>${badge(t.status || 'Ativo')}<button class="btn secondary" data-action="copy-url" data-url="${safe(t.url)}">Copiar URL</button><button class="btn ghost" data-action="make-rule" data-id="${t.id}">Usar no chatbot</button><button class="btn danger" data-action="delete-package" data-id="${t.id}">Excluir</button></div></div>`).join('')}</div>`
    : empty('?','Nenhuma URL de teste cadastrada','Cada cliente precisa cadastrar aqui as URLs do pr�prio provedor IPTV. Depois disso, ele pode transformar cada URL em uma regra do chatbot.','<button class="btn primary" data-action="new-package">Cadastrar URL do provedor</button>');
  return `<div class="section-head"><div><h2>Links de teste autom�tico</h2><p>Cadastre as URLs reais do provedor IPTV deste cliente. O sistema n�o usa links fixos: cada cliente informa os pr�prios links ou API.</p></div><button class="btn primary" data-action="new-package">+ Cadastrar URL do provedor</button></div><div class="banner"><div><h3>Como funciona</h3><p>O cliente cola a URL de teste do painel dele, define a palavra-chave, e depois usa �Usar no chatbot� para automatizar a resposta no WhatsApp.</p></div><a class="btn" href="#paineliptv">Configurar painel IPTV</a></div><div class="stats">${stat('URLs cadastradas',links.length,'Do cliente atual','?')}${stat('M�todo padr�o','POST','Ajust�vel por pacote','?','blue')}${stat('Resposta esperada','reply/text/JSON','Enviada no WhatsApp','WA')}${stat('Origem','Cliente','Provedor pr�prio','?','warn')}</div><article class="card"><div class="card-head"><h2>URLs do provedor IPTV</h2><button class="btn primary" data-action="new-package">+ Nova URL</button></div>${listHtml}</article><article class="card" style="margin-top:18px"><div class="card-head"><h2>Payload enviado para cada URL</h2></div><pre class="code">POST URL_CADASTRADA_PELO_CLIENTE
...
function openProviderModal() {
  const provider = state.iptvProvider || {};
  modalContent.innerHTML = `<h2>Configurar painel IPTV</h2><p class="subtitle">Essa configura��o pertence somente ao cliente logado.</p><form id="provider-form"><div class="form-grid"><div class="field"><label>Nome do painel/provedor</label><input class="input" name="name" value="${safe(provider.name || '')}" placeholder="Ex.: Alpha, QPanel, Sigma"></div><div class="field"><label>Tipo de integra��o</label><select class="select" name="mode"><option value="links" ${provider.mode !== 'api' ? 'selected' : ''}>Tenho links de teste</option><option value="api" ${provider.mode === 'api' ? 'selected' : ''}>Tenho API do painel</option></select></div><div class="field full"><label>URL base da API (opcional)</label><input class="input" name="apiBaseUrl" value="${safe(provider.apiBaseUrl || '')}" placeholder="https://painel-do-cliente.com/api"></div><div class="field"><label>Autentica��o</label><select class="select" name="authType"><option value="none" ${provider.authType === 'none' ? 'selected' : ''}>Nenhuma / link direto</option><option value="bearer" ${provider.authType === 'bearer' ? 'selected' : ''}>Bearer token</option><option value="apikey" ${provider.authType === 'apikey' ? 'selected' : ''}>API Key</option></select></div><div class="field"><label>Token/API Key</label><input class="input" name="maskedSecret" value="${provider.maskedSecret ? '********' : ''}" placeholder="Salvo com seguran�a no backend"></div><div class="field full"><label>Observa��es internas</label><textarea name="notes" rows="4" placeholder="Ex.: painel usa URLs /api/chatbot/...">${safe(provider.notes || '')}</textarea></div></div><div class="modal-actions"><button type="button" class="btn ghost" data-action="close-modal">Cancelar</button><button class="btn primary">Salvar painel</button></div></form>`;
  modal.hidden = false;
...
function updateAccountShell() {
  ...
  const name = tenant.name || profile.full_name || 'Minha opera��o';
  const roleMap = { owner: 'Assinante', admin: 'Administrador', reseller: 'Revendedor', agent: 'Atendente' };
  const role = roleMap[profile.role] || (productionMode() ? 'Assinante' : 'Demonstra��o');
  const mode = productionMode()
    ? (subscription.status === 'trial' && account.access?.trialDaysLeft ? `Teste gr�tis � ${account.access.trialDaysLeft} dia(s)` : 'Conta ativa')
...
function openRule(existing) {
  ...
  document.querySelector('#rule-form').onsubmit = async e => {
    ...
    try {
      ...
      save();
      modal.hidden = true;
      toast('Resposta autom�tica salva.');
      location.hash = '#chatbot';
      render();
    } catch (error) {
      toast(error instanceof Error ? error.message : 'Falha ao salvar resposta autom�tica.');
    }
  };
}
...
function watchConnection(instanceName) {
  ...
  const timer = setInterval(async () => {
    ...
    try {
      const response = await fetch(`/api/devices/${encodeURIComponent(instanceName)}`);
      const data = await response.json();
